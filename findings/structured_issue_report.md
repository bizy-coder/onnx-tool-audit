...
- **Repro**: `required` if the shown Inputs+Attrs is the only configuration that triggers the bug; `example (1/N)` if it's one of N distinct triggering configurations (we show the simplest).
